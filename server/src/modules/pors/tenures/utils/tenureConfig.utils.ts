...
export const parseBulkConfigUpdate = (
    ...
    if (item.parentRoleId !== undefined) {
        patch.parentRoleId = item.parentRoleId
            ? toObjectId(item.parentRoleId)
            : null;
    }
...
interface TreeNode {
    config: ITenureRoleConfig;
    children: TreeNode[];
}

export const buildConfigTree = (configs: ITenureRoleConfig[]) => {
    const nodes = new Map<string, TreeNode>();

    for (const config of configs) {
        nodes.set(config.roleId.toString(), { config, children: [] });
    }

    const roots: TreeNode[] = [];

    for (const config of configs) {
        const node = nodes.get(config.roleId.toString());
        if (!node) continue;

        const parentKey = config.parentRoleId?.toString();
